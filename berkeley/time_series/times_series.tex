...
\begin{itemize}
    ...
    % \item For a bivariate AR($p$) process, $\phi_{i,j}^k$ (the $(i,j)$th component of $\Phi_k$) 
    % is the influence of $Y_{j, t-k}$ on $Y_{i,t}, \, k=1, \ldots, p$.
...
\begin{itemize}
    ...
    % \item A VECM is a restriction on a VAR that imposes the condition that two variables can 
    % be cointegrated (they move together in some way that is stationary)
